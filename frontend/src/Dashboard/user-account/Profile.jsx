...
const Profile = ({user}) => {
  ...
  useEffect(() => {
    setFormData({ name: user.name, email: user.email, photo: user.photo, grade: user.grade })
  }, {user})
  ...
  const submitHandler = async event => {
    ...
    setLoading(false);
    // Check if responseData.message is available and contains a success message
    if (responseData.message) {
      toast.success(responseData.message);
    } else {
      // If message is not available, show a default success message
    ...
    <div className='mt-10'>
      <form onSubmit={submitHandler}>
        ...
        <div className='mb-5 flex items-center gap-3'>
          {formData.photo && selectedFile && <figure className='w-[60px] h[60px] rounded-full border-2 border-solid border-primaryColor flex items-center justify-center'>
